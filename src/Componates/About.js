...
export default function About(props) {
    ...
    let changeTheme = () => {
            if(accordionStyle.backgroundColor === "#2C3333"){
                changeStyle({
    ...
    const[btnText,changeText] = useState("white");
   
    const [accordionStyle, changeStyle] = useState({
        backgroundColor: '#2C3333',
        color: "white"
    })
    return (
        <>
            <div className='container-fluid' style={props.mystyle}>
                ...
                <button className="btn btn-light my-2 border border-2 border-dark px-4 text-dark" onClick={changeTheme}>{btnText}</button>
            </div>
        </>
